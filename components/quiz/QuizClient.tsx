...
const QuizClient = ({ quiz }: QuizClientProps) => {
  ...
  if (questions.length === 0) {
    return (
      <PaddingContainer className="py-16 text-center">
  ...
  if (!hasStarted) {
    return (
      <PaddingContainer className="py-16 max-w-2xl mx-auto min-h-screen flex flex-col justify-center">
        <div className="bg-card border rounded-3xl p-8 md:p-12 shadow-xl text-center">
          ...
          <div className="grid grid-cols-2 gap-4 mb-8 text-left max-w-md mx-auto">
             <div className="bg-secondary/30 p-4 rounded-xl">
                <span className="block text-sm text-muted-foreground uppercase tracking-wider font-semibold">Total Questions</span>
                <span className="text-2xl font-bold">{questions.length}</span>
             </div>
             <div className="bg-secondary/30 p-4 rounded-xl">
                <span className="block text-sm text-muted-foreground uppercase tracking-wider font-semibold">Total Time</span>
                <span className="text-2xl font-bold">{quiz.total_time || "N/A"} Mins</span>
             </div>
             <div className="bg-secondary/30 p-4 rounded-xl">
                <span className="block text-sm text-muted-foreground uppercase tracking-wider font-semibold">Total Marks</span>
                <span className="text-2xl font-bold">{quiz.total_marks || questions.length}</span>
             </div>
             <div className="bg-secondary/30 p-4 rounded-xl">
                <span className="block text-sm text-muted-foreground uppercase tracking-wider font-semibold">Passing Marks</span>
                 {/* Assuming 50% passing for now or add to existing logic later */}
                <span className="text-2xl font-bold">{Math.ceil((quiz.total_marks || questions.length) * 0.4)}</span>
             </div>
          </div>
          ...
          {quiz.notes && (
            <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 p-6 rounded-xl mb-8 text-left">
              ...
              <div className="text-blue-700 dark:text-blue-400 prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: quiz.notes }} />
            </div>
          )}
  ...
  if (isFinished) {
    ...
    return (
      <PaddingContainer className="py-16 max-w-3xl mx-auto">
        <div className="bg-card border rounded-2xl p-8 shadow-lg text-center mb-8">
          <div className="mb-6">
            <h1 className="text-3xl font-bold mb-2">Quiz Completed!</h1>
            <p className="text-muted-foreground">You have finished {quiz.title}</p>
          </div>
    ...
    <PaddingContainer className="py-12 max-w-7xl mx-auto min-h-screen flex flex-col">
      ...
      <div className="mb-8 max-w-7xl mx-auto w-full">
        <div className="bg-card border shadow-sm rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
           ...
           <div className={`w-full md:w-auto flex items-center justify-center gap-3 px-6 py-3 rounded-xl border-2 transition-colors duration-300 ${timeLeft < 60 ? 'border-red-500 bg-red-50 text-red-600 animate-pulse' : 'border-primary/20 bg-primary/5 text-primary'}`}>
            ...
            {currentQuestion?.multiple_questions.map((option, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-medium text-lg md:text-xl relative z-10">{option.option}</span>
                </button>
              );
            })}
          </div>
